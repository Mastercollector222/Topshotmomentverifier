...
export async function getUserMoments(address) {
  ...
  const script = `
    ...
    access(all) fun main(user: Address): [MomentData] {
      ...
      if let collectionRef = account.capabilities.get<&{TopShot.MomentCollectionPublic, NonFungibleToken.CollectionPublic}>(/public/MomentCollection).borrow() {
...
export async function verifyMomentOwnership(address, momentId) {
  ...
  const script = `
    ...
    access(all) fun main(user: Address, momentID: UInt64): Bool {
      ...
      if let collection = account.capabilities.get<&{TopShot.MomentCollectionPublic, NonFungibleToken.CollectionPublic}>(/public/MomentCollection).borrow() {
